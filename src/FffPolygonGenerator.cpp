...
bool FffPolygonGenerator::sliceModel(MeshGroup* meshgroup, TimeKeeper& timeKeeper, SliceDataStorage& storage) /// slices the model
{
    ...
    {
        spdlog::error("Layer height {} is disallowed.", layer_thickness);
        return false;
    }
...
void FffPolygonGenerator::slices2polygons(SliceDataStorage& storage, TimeKeeper& time_keeper)
{
    ...
    {
        spdlog::info("Stopping process because there are no non-empty layers.");
        return;
    }
    ...
    {
        spdlog::info("Processing platform adhesion");
...
#pragma omp parallel for default(none) shared(mesh_layer_count, storage, mesh, inset_skin_progress_estimate, processed_layer_count) schedule(dynamic)
    ...
    {
        spdlog::debug("Processing insets for layer {} of {}", layer_number, mesh_layer_count);
...
#pragma omp for schedule(dynamic)
        ...
        for (int layer_number = 0; layer_number < static_cast<int>(mesh.layers.size()); layer_number++)
        {
            spdlog::debug("Processing skins and infill layer {} of {}", layer_number, mesh_layer_count);
...
void FffPolygonGenerator::processDerivedWallsSkinInfill(SliceMeshStorage& mesh)
{
    ...
    )
    {
        ...
        else
        {
            ...
            {
                spdlog::error("Cannot find density image \'{}\'.", cross_subdivision_spec_image_file.c_str());
            }
