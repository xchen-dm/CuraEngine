...
void CommandLine::sendPrintTimeMaterialEstimates() const
{
    ...
    spdlog::info("Total print time: {}", sum);
    ...
    for (size_t argument_index = 2; argument_index < arguments.size(); argument_index++)
    {
        ...
        if (argument[0] == '-') //Starts with "-".
        {
            if (argument[1] == '-') //Starts with "--".
            {
                ...
                {
                    spdlog::error("Unknown option: {}", argument.c_str());
                }
            }
            else //Starts with "-" but not with "--".
            {
                ...
                switch(argument[1])
                {
                    ...
                    case 'j':
                    {
                        ...
                        {
                            spdlog::error("Failed to load JSON file: {}", argument.c_str());
                            exit(1);
                        }
                    ...
                    case 'l':
                    {
                        ...
                        {
                            spdlog::error("Failed to load model: {}. (error number {})", argument.c_str(), errno);
                            exit(1);
                        }
                        else
                        {
                    ...
                    case 's':
                    {
                        ...
                        {
                            spdlog::error("Missing value in setting argument: -s {}", argument.c_str());
                            exit(1);
                        }
...
int CommandLine::loadJSON(const std::string& json_filename, Settings& settings)
{
    ...
    if (!file)
    {
        spdlog::error("Couldn't open JSON file: {}", json_filename.c_str());
        return 1;
    }
    ...
    {
        spdlog::error("Error parsing JSON (offset {}): {}", static_cast<unsigned int>(json_document.GetErrorOffset()), GetParseError_En(json_document.GetParseError()));
        return 2;
    }
...
void CommandLine::loadJSONSettings(const rapidjson::Value& element, Settings& settings)
{
    for (rapidjson::Value::ConstMemberIterator setting = element.MemberBegin(); setting != element.MemberEnd(); setting++)
    {
        ...
        {
            spdlog::error("JSON setting {} is not an object!", name.c_str());
            continue;
        }
        ...
        else //Only process leaf settings. We don't process categories or settings that have sub-settings.
        {
            ...
            {
                spdlog::warn("JSON setting {} has no default_value!", name.c_str());
                continue;
            }
            ...
            {
                spdlog::warn("Unrecognized data type in JSON setting {}", name.c_str());
                continue;
            }
...
const std::string CommandLine::findDefinitionFile(const std::string& definition_id, const std::unordered_set<std::string>& search_directories)
{
    ...
    spdlog::error("Couldn't find definition file with ID: {}", definition_id.c_str());
    return std::string("");
}
